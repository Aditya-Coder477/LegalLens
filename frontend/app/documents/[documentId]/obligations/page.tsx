...
export default function DocumentObligationsPage() {
  ...
  const loadObligations = async () => {
    setLoading(true);
    try {
      const res = await LegalLensAPI.extractObligations(documentId);
      setResponse(res);
    } catch (err) {
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex items-center justify-between">
        ...
        >
          ...
          <span>Re-extract Obligations</span>
        </button>
      </div>
      ...
      {loading && !response && (
        <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center shadow-xs">
          ...
          <h3 className="text-xs font-bold text-slate-800">Extracting Party Commitments</h3>
          <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1">
            Parsing covenants, duties, conditions precedent, and reciprocal obligations...
          </p>
        </div>
      )}

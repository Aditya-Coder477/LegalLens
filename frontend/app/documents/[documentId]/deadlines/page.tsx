...
import { ArrowLeft, Clock, Sparkles, Loader2, RefreshCw, Calendar } from 'lucide-react';
...
export default function DocumentDeadlinesPage() {
  ...
  const loadDeadlines = async () => {
    setLoading(true);
    try {
      const res = await LegalLensAPI.extractDeadlines(documentId);
      setResponse(res);
    } catch (err) {
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="text-base font-bold text-slate-900">
              Critical Notice Periods, Milestones & Timelines
            </h1>
          </div>
        </div>
        ...
        >
          ...
          <span>Re-extract Deadlines</span>
        </button>
      </div>
      ...
      {loading && !response && (
        <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center shadow-xs">
          ...
          <h3 className="text-xs font-bold text-slate-800">Compiling Timelines</h3>
          <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1">
            Identifying notice timeframes, cure periods, limitation dates, and return deadlines...
          </p>
        </div>
      )}
